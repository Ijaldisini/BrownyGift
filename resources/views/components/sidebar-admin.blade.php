<aside class="w-64 bg-white shadow-2xl fixed h-screen z-50 border-r-2 border-pink-100 overflow-y-auto">
    ...
    <nav class="mt-6 flex flex-col">
        ...
        <a href="{{ route('admin.pesanan.index') }}"
            class="{{ $defaultClass }} {{ request()->routeIs('admin.pesanan.*') ? $activeClass : '' }}">
            <i class="bi bi-receipt text-lg mr-3"></i>
            Pesanan
        </a>

        <a href="{{ route('admin.laporan.laporan') }}"
            class="{{ $defaultClass }} {{ request()->routeIs('admin.laporan.*') ? $activeClass : '' }}">
        ...
        <!-- Logout -->
        <a href="{{ url('/logout') }}" onclick="return confirm('Yakin ingin keluar dari admin panel?')"
