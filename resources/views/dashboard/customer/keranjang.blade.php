...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-md-3 sidebar">
            ...
            <a href="{{ route('customer.keranjang') }}" class="active"><i class="fas fa-shopping-cart"></i> Keranjang</a>
        ...
        <div class="col-md-9 main-content">
            ...
            <div class="cart-container">
                <!-- Empty Cart State -->
                <div class="empty-cart" id="emptyCart" style="{{ $cartItems->isEmpty() ? '' : 'display: none;' }}">
                ...
                <div id="cartItems" style="{{ $cartItems->isEmpty() ? 'display: none;' : '' }}">
                    @foreach($cartItems as $item)
                    <!-- Cart Item -->
                    <div class="cart-item" data-id="{{ $item->id_keranjang }}">
                        <div class="cart-item-image">
                            <img src="{{ asset('storage/products/' . $item->produk->gambar_produk) }}" alt="{{ $item->produk->nama_produk }}">
                        </div>
                        <div class="cart-item-details">
                            <div class="cart-item-category">Product</div>
                            <div class="cart-item-title">{{ $item->produk->nama_produk }}</div>
                            <div class="cart-item-price">Rp {{ number_format($item->produk->harga_produk, 0, ',', '.') }}</div>
                        </div>
                        <div class="cart-item-actions">
                            <div class="quantity-control">
                                ...
                                <span id="qty-{{ $item->id_keranjang }}">{{ $item->quantity }}</span>
                                <button onclick="updateQty({{ $item->id_keranjang }}, 1)">
                                    <i class="fas fa-plus"></i>
                                </button>
                            </div>
                            <form action="{{ route('customer.keranjang.remove', $item->id_keranjang) }}" method="POST" onsubmit="return confirm('Hapus produk?');">
                                @csrf
                                @method('DELETE')
                    ...
                    <div class="cart-summary">
                        ...
                        <div class="summary-row">
                            <span>Subtotal (<span id="totalItems">{{ $cartItems->sum('quantity') }}</span> item)</span>
                            <span id="subtotal">Rp {{ number_format($cartItems->sum(function($item){ return $item->produk->harga_produk * $item->quantity; }), 0, ',', '.') }}</span>
                        </div>
                        <div class="summary-row">
                        ...
                        <div class="summary-row total">
                            <span>Total</span>
                            <span id="total">Rp {{ number_format($cartItems->sum(function($item){ return $item->produk->harga_produk * $item->quantity; }) + 15000, 0, ',', '.') }}</span>
                        </div>
                        <button class="btn-checkout" onclick="checkout()">
                            <i class="fas fa-credit-card me-2"></i> Checkout
                        </button>
                        <div class="text-center">
                            <a href="{{ route('customer.produk') }}" class="btn-continue">
...
<script>
    ...
    function updateQty(id, change) {
        ...
        fetch('{{ route("customer.keranjang.update") }}', {
            method: 'PUT',
            headers: {
    ...
    function checkout() {
        if ({{ $cartItems->isEmpty() ? 'true' : 'false' }}) {
